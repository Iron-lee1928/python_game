...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        ball_width = ball_size[0]  # x 값
        ball_height = ball_size[1]  # y 값
        # 가로 위치
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                ball_to_remove = ball_idx  # 해당 공 업애기 위한 값 설정

                # 가장 작은 크기의 공이 아니라면 다음 단계 공으로 나눠주기
                if ball_img_idx < 3:
                    # 현재 공 크기 정보를 가지고 옴
                    ...
                    balls.append({
                        ...
                        "img_idx":
                        ball_img_idx + 1,  #공의 이미지 인덱스 번호 
                        "to_x":
                        -3,  #x축 이동방향, -3이면 왼쪽, 3 이면 오른쪽
                        "to_y":
                        -6,  #y축 이동방향,
                        "init_spd_y":
...
# 게임 오버 메시지
msg = game_font.render(game_result, True, (255, 255, 0))  # 노란색 글씨
msg_rect = msg.get_rect(center=(int(screen_width / 2), int(screen_height / 2)))
screen.blit(msg, msg_rect)
pygame.display.update()

# 2초 딜레이 (게임이 바로 꺼지는걸 방지)
